    ...
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div>
            <p className="font-semibold text-stone-800 text-sm">{maskName(order.customer_name)}</p>
            <p className="text-stone-400 text-xs mt-0.5">{maskPhone(order.whatsapp)}</p>
          </div>
          <span className={cn(
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50 px-4 text-center">
        <div>
          ...
          <Link to="/" className="text-amber-600 text-sm mt-2 inline-block hover:underline">← Back to home</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-6">
          <Link to="/" className="text-xs text-stone-400 hover:text-stone-600 mb-3 inline-flex items-center gap-1">← Home</Link>
          <h1 className="text-2xl font-bold text-stone-800">{session.title}</h1>
          <p className="text-stone-500 text-sm mt-0.5">Pick-up: {formatDate(session.fulfillment_date)}</p>
        </div>
          ...
          <div className="flex-1 max-w-[120px]">
            <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: orders.length ? `${(fulfilled / orders.length) * 100}%` : '0%' }}
              />
            </div>
            <p className="text-xs text-stone-400 mt-1 text-right">{orders.length ? Math.round((fulfilled / orders.length) * 100) : 0}% ready</p>
          </div>
        </div>
        ...
        {orders.length === 0 ? (
          <div className="text-center py-16 text-stone-400">
            ...
            <Link to={`/order/${session.id}`} className="inline-block mt-4 bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded-xl px-5 py-2.5 text-sm transition-colors">
              Order Now →
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-xs font-bold uppercase tracking-widest text-stone-400 mb-2">Order List</p>
            {orders.map((order, i) => (
        ...
        <p className="text-center text-xs text-stone-300 mt-8">Refreshes automatically every 30 seconds</p>
      </div>
    </div>
  )
}
